...
> **它分五小块：**
> ① **这一关要回答的问题**；
> ② ⭐ **理论背景** —— **这一节要够厚**：这一关的零件是什么、规则是什么、语义怎么定义、
>    源码里是怎么回事。**写成表格和定义，不要长段落**，因为他要**边答题边翻**；
> ③ **这次会动的旋钮**（从什么 → 到什么 → 管哪一格）+ **API 形状**（英文我翻好）；
> ④ **和前面哪几关连着**；
> ⑤ **实验预告**：做哪几个实验、每个动哪根旋钮。**不给结果。**
>
> ⚠️ **边界（一句话判据）**：
> **把那句话换一个旋钮，它还对吗？**
> **还成立 = 理论 → 写。只对这一个场景成立 = 结果 → 一个字都不写。**
>
> | 例 | 是理论还是结果 | 写不写 |
> |---|---|---|
> | "可重入组 = 允许重叠，**不是**允许并行" | 理论（换旋钮照样成立） | ✅ 写 |
> | "可重入组动的是『锁』那一格，把它变成『没有』" | 理论（换个场景也成立） | ✅ 写 |
> | "两个可重入定时器 + 多线程容器 = **会夹**" | **结果** | ❌ **不写** |
>
> **理论给的是前提，推出来的那一步必须是他自己的** —— 因为让他能推、而不是能背，正是这一节存在的理由。
