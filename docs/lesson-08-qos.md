...
### ❌ 坑 5：最后一步被 `colcon test` 抓到的行尾空格

收尾跑全量测试，**唯一一条真失败**是：

```text
AssertionError: Found 1 code style errors / warnings:
  ./setup.py:48:54: W291 trailing whitespace
assert 1 == 0
```

第 48 行正是本关新加的那行 —— `'qos_talker = hello_ros.qos_talker:main',` **后面多了几个空格**。
`cat -A` 一看便知（行尾是 `,` + 空格 + `$`）。

**这不是"没写好"，是"编辑器看不见"。** 行尾空格在屏幕上和换行长得一模一样，
只有 `cat -A` 或 lint 能看见。

> **现实含义**：这个仓库的 `colcon test` **真的会拦住东西**。
> 只要你敲了 `colcon test`，它在替你看那些你眼睛看不见的字符。
> **这也是"贴半截输出"的又一个变种** —— 报错里 `./setup.py:48:54` **连列号都给了**
> （第 54 列，就是那行结尾），照着一行一行数过去就行。
